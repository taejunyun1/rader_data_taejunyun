...
- 착즙은 문서 목차와 읽기 큐를 제공하며, OpenAlex 검증 전 큐 항목의 저장소 승격을 막는다. `반대 관점 포함`은 기본 켜짐이고 실행 전에 끌 수 있으며, 켜진 Counter는 정면 반대 명제와 정합성 검증 상태를 함께 표시한다. 2026-09-04부터 기본 프롬프트 `distill-v3-layered`는 기존 요약과 함께 항목별 상세 근거·불확실성·다음 확인/조사 방법/예상 결과/작업 절차를 한 번에 생성한다. 상세는 `distill_sessions.output_json.details`의 선택 필드로 보존하며, 기존 세션은 그대로 읽힌다. Radar UI는 요약을 먼저 보여주고 `근거와 맥락 보기`에서만 상세를 펼친다. 상세가 참조한 source ID는 세션 조회 API의 `detailSources`에서 현재 저장소 존재 여부와 제목을 확인하며, 활성 자료만 저장소 상세로 이동할 수 있다. 마크다운 내보내기는 상세를 포함하지만 홈페이지 `현재 연구` projection은 기존 상한·필드만 사용해 상세를 공개하지 않는다. 별도 D1 migration은 필요하지 않다.
