...
- 발견과 저장소의 데스크톱 읽기 작업공간은 목록과 읽기 pane을 동시에 유지하고 각각 독립적으로 스크롤한다. 자료를 바꾸면 읽기 pane만 맨 위로 이동하며 목록 위치는 보존한다. 900px 이하에서는 목록과 읽기를 한 번에 하나만 표시하고 `목록으로`로 돌아간다. 자료 선택은 읽기만 열고, 판단 바텀시트는 `판단하기` 또는 `판단 변경`을 눌렀을 때만 연다.
