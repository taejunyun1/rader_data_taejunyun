...
- 기존 `discovery:*` 및 `homepage-reading` 웹 자료 재수집은 `POST /api/settings/backfill-discovery`의 수동·중복 방지 batch로만 수행한다. active version이 `FULLTEXT`가 아니거나 1,000자 미만인 자료를 오래된 순서로 최대 10건 선택하며, 공개 정적 HTML/PDF만 수집하고 자동 backfill/acquisition cron은 없다. 기존 주간 Discovery 후보 탐색 cron은 별도 동작이다.
