...
export async function runRadarSynthesis(env: Env, period: RadarPeriod): Promise<{ snapshotId: string; synthesis: unknown }> {
  const result = await synthesizeRadar(env, period);
