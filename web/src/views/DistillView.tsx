...
import type { DistillOutput } from "@radar/shared/distill";
...
import DistillDetailDisclosure from "../components/distill/DistillDetailDisclosure";
import DistillDetailContent from "../components/distill/DistillDetailContent";
import type { DistillDetailSourceRef } from "../components/distill/DistillDetailContent";
...
export default function DistillView({ onJobCreated, focusSessionId, onFocusConsumed, onOpenReservoir }: { onJobCreated?: () => Promise<void>; focusSessionId?: string; onFocusConsumed?: () => void; onOpenReservoir?: (sourceId: string) => void }) {
  ...
  return <div className="view-stack">
    ...
    {!data || !output ? <StatusMessage kind="empty" title="아직 착즙 결과가 없습니다" description="저장소의 자료를 바탕으로 첫 착즙을 시작하세요." /> : <div className="distill-document-layout"><DocumentOutline sections={[...outline, { id: "counter", label: "정면 반대 관점" }]} /><main className="distill-document"><p className="document-meta">생성 {new Date(data.session.createdAt).toLocaleString("ko-KR")} · 사용 모델 {data.session.modelVersion} · 비용 ${data.session.costUsd.toFixed(4)} · 자료 {data.session.sourcesUsed?.length ?? 0}개 · {data.session.counterEnabled === false ? "반대 관점 제외" : "반대 관점 포함"}</p>
      ...
      {output.thoughts_fragments.length > 0 && <section id="thoughts" className="distill-section"><p className="reading-section__label">생각의 조각</p>{output.details && <p className="distill-detail-intro">요약은 먼저 읽고, 항목별 근거와 다음 확인 지점은 필요할 때 펼쳐보세요.</p>}{output.thoughts_fragments.map((item, index) => { const detail = output.details?.thoughts.find((candidate) => candidate.summaryIndex === index); return <div className="distill-summary-item" key={`${item}-${index}`}><p className="distill-copy">{item}</p>{detail && <DistillDetailDisclosure><DistillDetailContent fields={[{ label: "근거", value: detail.rationale }, { label: "불확실성", value: detail.uncertainty }, { label: "다음 확인", value: detail.nextCheck }]} sourceIds={detail.sourceIds} sourceRefs={data.detailSources ?? []} onOpenReservoir={onOpenReservoir} /></DistillDetailDisclosure>}</div>; })}</section>}
      {output.questions.length > 0 && <section id="questions" className="distill-section"><p className="reading-section__label">질문</p>{output.questions.map((item, index) => { const detail = output.details?.questions.find((candidate) => candidate.summaryIndex === index); return <div className="distill-summary-item" key={`${item}-${index}`}><p className="reading-question"><span>{String(index + 1).padStart(2, "0")}</span>{item}</p>{detail && <DistillDetailDisclosure><DistillDetailContent fields={[{ label: "지금 묻는 이유", value: detail.whyNow }, { label: "조사 방법", value: detail.method }, { label: "필요한 증거", value: detail.evidenceNeeded }]} sourceIds={detail.sourceIds} sourceRefs={data.detailSources ?? []} onOpenReservoir={onOpenReservoir} /></DistillDetailDisclosure>}</div>; })}</section>}
      ...
      {output.research_gaps.length > 0 && <section id="research-gaps" className="distill-section"><p className="reading-section__label">연구 공백</p>{output.research_gaps.map((item, index) => { const detail = output.details?.researchGaps.find((candidate) => candidate.summaryIndex === index); return <div className="distill-summary-item" key={`${data.session.id}-research-gap-${index}`}><p className="distill-copy">{item.gap}{item.kind && <span className="pill">{labelOf(RESEARCH_GAP_LABELS, item.kind)}</span>}</p>{detail && <DistillDetailDisclosure><DistillDetailContent fields={[{ label: "진단", value: detail.diagnosis }, { label: "연구 방법", value: detail.researchMethod }]} sourceIds={detail.sourceIds} sourceRefs={data.detailSources ?? []} onOpenReservoir={onOpenReservoir} /></DistillDetailDisclosure>}</div>; })}</section>}
      {output.research_directions.length > 0 && <section id="directions" className="distill-section"><p className="reading-section__label">연구 방향</p>{output.research_directions.map((item, index) => { const detail = output.details?.researchDirections.find((candidate) => candidate.summaryIndex === index); return <div className="distill-summary-item" key={`${item}-${index}`}><p className="distill-copy">{item}</p>{detail && <DistillDetailDisclosure><DistillDetailContent fields={[{ label: "근거", value: detail.rationale }, { label: "방법", value: detail.method }, { label: "예상 결과", value: detail.expectedOutcome }]} sourceIds={detail.sourceIds} sourceRefs={data.detailSources ?? []} onOpenReservoir={onOpenReservoir} /></DistillDetailDisclosure>}</div>; })}</section>}
      {output.artwork_directions.length > 0 && <section id="artwork" className="distill-section"><p className="reading-section__label">작업 방향</p>{output.artwork_directions.map((item, index) => { const detail = output.details?.artworkDirections.find((candidate) => candidate.summaryIndex === index); return <div className="distill-summary-item" key={`${item}-${index}`}><p className="distill-copy">{item}</p>{detail && <DistillDetailDisclosure><DistillDetailContent fields={[{ label: "근거", value: detail.rationale }, { label: "재료", value: detail.materials.join(", ") }, { label: "절차", value: detail.procedure }, { label: "관찰", value: detail.observation }]} sourceIds={detail.sourceIds} sourceRefs={data.detailSources ?? []} onOpenReservoir={onOpenReservoir} /></DistillDetailDisclosure>}</div>; })}</section>}
