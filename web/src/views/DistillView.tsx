...
interface SessionData { session: { id: string; redistillOf: string | null; modelVersion: string; promptVersion: string; costUsd: number; createdAt: string; sourcesUsed: { id: string; title: string }[] | null; output: DistillOutput | null; critic: { warnings: { category: string; note: string }[]; overall: string } | null; counter: { axes: { from: string; to: string; rationale: string }[]; suggestions: { direction: string; grounding: { name: string; kind: string; note: string }[] }[] } | null; }; readingQueue: QueueItem[]; researchGaps: { id: string; gap: string; kind: string | null }[]; }
interface SessionListItem { id: string; redistillOf: string | null; costUsd: number; createdAt: string; }
...
export default function DistillView() {
  ...
  const distillBusy = busy || tasks.some((task) => (task.label === "Distill" || task.label === "Re-distill") && task.status === "running");
  const runningTaskMsg = tasks.find((task) => (task.label === "Distill" || task.label === "Re-distill") && task.status === "running")?.message;

  async function openSession(id: string) { const response = await fetch(`/api/distill/sessions/${id}`); if (response.ok) setData(await response.json() as SessionData); }
  ...
  async function runDistill(redistillOf?: string) {
    ...
    setBusy(true);
    try { await runTask(redistillOf ? "Re-distill" : "Distill", async (setTaskMsg, setProgress) => { setTaskMsg("읽기 맥락을 정리하는 중"); setProgress(20); const response = await fetch("/api/distill/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(redistillOf ? { redistillOf, keepElements: kept } : {}) }); const result = await response.json() as { sessionId?: string; error?: string }; if (!response.ok || !result.sessionId) throw new Error(result.error ?? "착즙 실행에 실패했습니다."); setProgress(75); setKept([]); await openSession(result.sessionId); await loadSessions(); setTaskMsg("착즙 완료"); setMsg("새 착즙 결과를 준비했습니다."); fetch("/api/distill/budget").then((item) => item.json() as Promise<Budget>).then(setBudget).catch(() => undefined); }); } catch (error) { setMsg(error instanceof Error ? error.message : "착즙에 실패했습니다."); } finally { setBusy(false); }
  }
  ...
  return <div className="view-stack">
    <PageHeader title="착즙" description="보존한 자료에서 다음 읽기와 연구 방향을 편집합니다." primaryAction={<button className="ui-button" disabled={distillBusy} onClick={() => void runDistill()}>{distillBusy ? "착즙 중…" : data ? "새로 착즙하기" : "첫 착즙 시작"}</button>} />
    ...
    {!data || !output ? <StatusMessage kind="empty" title="아직 착즙 결과가 없습니다" description="저장소의 자료를 바탕으로 첫 착즙을 시작하세요." /> : <div className="distill-document-layout"><DocumentOutline sections={outline} /><main className="distill-document"><p className="document-meta">{new Date(data.session.createdAt).toLocaleString("ko-KR")} · {data.session.modelVersion} · ${data.session.costUsd.toFixed(4)} · 자료 {data.session.sourcesUsed?.length ?? 0}개</p>
      ...
      <section id="reading-queue" className="distill-section"><div className="distill-section__heading"><p className="reading-section__label">다음 읽기</p><button className="ui-button-secondary" onClick={() => void verifyQueue()}>접근 경로 다시 확인</button></div>{data.readingQueue.length === 0 ? <p className="distill-copy">추천된 다음 읽기가 없습니다.</p> : data.readingQueue.map((item) => <article className="queue-card" key={item.id}><div><h3>{item.sourceUrl ? <a href={item.sourceUrl} target="_blank" rel="noreferrer">{item.title}</a> : item.title}</h3><p>{item.author ?? "저자 정보 없음"} · {item.priority} · {item.verified ? "확인됨" : "미확인"}</p>{item.whyRead && <span>{item.whyRead}</span>}{item.relatedQuestion && <blockquote>{item.relatedQuestion}</blockquote>}</div><button className="ui-button-secondary" disabled={!item.verified} title={item.verified ? "저장소에 보관" : "먼저 접근 경로를 확인하세요"} onClick={() => void importQueueItem(item)}>{item.verified ? "저장소에 보관" : "확인 필요"}</button></article>)}</section>
      {data.researchGaps.length > 0 && <section id="research-gaps" className="distill-section"><p className="reading-section__label">연구 공백</p>{data.researchGaps.map((item) => <p className="distill-copy" key={item.id}>{item.gap}{item.kind && <span className="pill">{item.kind}</span>}</p>)}</section>}
