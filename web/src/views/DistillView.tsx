...
export default function DistillView({ onJobCreated, focusSessionId, onFocusConsumed, onOpenReservoir }: { onJobCreated?: () => Promise<void>; focusSessionId?: string; onFocusConsumed?: () => void; onOpenReservoir?: (sourceId: string) => void }) {
  ...
  return <div className="view-stack">
    ...
    {!data || !output ? <StatusMessage kind="empty" title="아직 착즙 결과가 없습니다" description="저장소의 자료를 바탕으로 첫 착즙을 시작하세요." /> : <div className="distill-document-layout"><DocumentOutline sections={[...outline, { id: "counter", label: "정면 반대 관점" }]} /><main className="distill-document"><p className="document-meta">생성 {new Date(data.session.createdAt).toLocaleString("ko-KR")} · 사용 모델 {data.session.modelVersion} · 비용 ${data.session.costUsd.toFixed(4)} · 자료 {data.session.sourcesUsed?.length ?? 0}개 · {data.session.counterEnabled === false ? "반대 관점 제외" : "반대 관점 포함"}</p>
      ...
      {data.researchGaps.length > 0 && <section id="research-gaps" className="distill-section"><p className="reading-section__label">연구 공백</p>{data.researchGaps.map((item) => { const summaryIndex = output.research_gaps.findIndex((candidate) => candidate.gap === item.gap); const detail = summaryIndex >= 0 ? output.details?.researchGaps.find((candidate) => candidate.summaryIndex === summaryIndex) : undefined; return <div className="distill-summary-item" key={item.id}><p className="distill-copy">{item.gap}{item.kind && <span className="pill">{labelOf(RESEARCH_GAP_LABELS, item.kind)}</span>}</p>{detail && <DistillDetailDisclosure><DistillDetailContent fields={[{ label: "진단", value: detail.diagnosis }, { label: "연구 방법", value: detail.researchMethod }]} sourceIds={detail.sourceIds} sourceRefs={data.detailSources ?? []} onOpenReservoir={onOpenReservoir} /></DistillDetailDisclosure>}</div>; })}</section>}
