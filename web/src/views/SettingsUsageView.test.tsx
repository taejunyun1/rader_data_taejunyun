...
describe("SettingsView", () => { it("uses Korean labels for research controls", async () => { render(<SettingsView />); expect(await screen.findByRole("heading", { name: "연구 성향" })).toBeInTheDocument(); expect(screen.getByText("깊은 연구")).toBeInTheDocument(); }); it("shows two selectable AI model roles", async () => { render(<SettingsView />); expect(await screen.findByLabelText("기본 모델")).toHaveValue("gpt-5-mini"); expect(screen.getByLabelText("상위 통합·반론 검증 모델")).toHaveValue("gpt-5.4-mini"); }); });
