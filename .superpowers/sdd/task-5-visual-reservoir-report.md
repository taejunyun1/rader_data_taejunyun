...
- Made PDF continuation terminal-aware: `SUCCEEDED` units are skipped, failed/partial inputs remain retryable until success, and existing candidate analysis/transform state is reused without duplicate work.
- Kept permitted/personal `DUPLICATE` candidates on metadata/provenance plus `DUPLICATE_OF` only; only `SELECTED` and `REVIEW` candidates cross the archival crop and `VISUAL_TRANSFORM` boundary.
...
- `pnpm --dir web exec vitest run src/lib/visualAssets.test.ts src/lib/deepAnalysis.test.ts` — 42 tests passed
- `pnpm typecheck`

Both commands passed on 2026-08-26 after the resume/duplicate-gate reviewer-fix code changes.
...
- A failed or partial unit retains its temp page object for workflow retry; the successful-unit cleanup runs only after the unit is marked terminal-success. Inputs already deleted by earlier pre-fix runs require a fresh page upload.
